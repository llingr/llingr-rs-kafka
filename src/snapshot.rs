...
//! on an operational HTTP route) should use the raw JSON accessor on the
//! engine handle; this module is for programmatic access.
//!
...
impl Snapshot {
    ...
    ///
    /// Unknown fields are tolerated (a newer engine may add diagnostics);
    /// a missing or malformed known field is an error.
...
mod tests {
    ...
    /// llingr-demux v0.12.0 (demux/metrics/snapshot), not hand-written.
    /// Regenerate against the new pin whenever the engine version bumps.
    ...
    /// A missing known section is an error, not a silent default: an engine
    /// document that lost a section indicates a contract change that must
