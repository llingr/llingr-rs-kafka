...
//! The pure-Go client speaks Kafka and every Kafka-compatible broker,
//! RedPanda and Amazon MSK included. Per-key concurrent processing without
...
//!   engine registers a loader initialiser: the Go runtime comes up before
//!   `main()` runs, not at `build()`, bringing its signal handlers for
//!   SIGSEGV, SIGBUS, SIGFPE, SIGPROF, and SIGURG with it. The engine never
//!   claims SIGINT or SIGTERM, so registering handlers for those works at
//!   any point; `signal-hook`, for example, chains to any pre-existing
//!   handler. Do not install non-chaining handlers for the fault signals,
//!   and anything touching them must use `SA_ONSTACK`.
//!
//! - **Do NOT call stop() from a signal handler.** Go code is never
//!   async-signal-safe: calling stop() or any engine function from inside a
//!   signal handler can deadlock or crash the process. Use
//!   `signal_hook::flag` to set an atomic flag, then poll it from a normal
//!   thread and call [`stop`](Llingr::stop).
//!
...
//!
//! - **musl/Alpine is not supported yet**: an upstream Go limitation; the
//!   build script fails with the full explanation and links. Use a glibc
//!   image such as Debian or Ubuntu; a scratch image works because the
//!   binary is static.
//!
...
//!   bytes*: silent wrong data, not necessarily a crash. Copy out anything
//!   you need to keep with `to_vec()` or `to_string()`.
//!
...
//!   drain; that is *safe*, because uncommitted offsets are redelivered at
//!   least once on restart, but wasteful. Never call `stop()` from inside a
//!   process or dead-letter handler; signal another thread.
//!
//! - **[`emergency_stop`](Llingr::emergency_stop) abandons in-flight work**:
//!   no drain, no final commit, and duplicates on restart are expected; the
...
//!
//! - **Liveness is your responsibility.** If a handler stalls, nothing
...
// Coverage-focused unit tests over pub(crate) internals.
#[cfg(test)]
mod engine_coverage_tests;
#[cfg(test)]
mod options_auth_tests;
#[cfg(test)]
mod options_gcp_tests;
#[cfg(test)]
...
// suffices. Deliberately NOT re-exported: nexus's per-message metrics packet
// type `Metrics` with its `MetricsHandler` trait, the bandwidth equivalents,
// and the logger plumbing. The Prometheus integration consumes the packet
// type internally, the root name `Metrics` belongs to the activation type,
// and engine logs always flow to the `log` facade.
