...
and one of which we do not hit at all.
...
*host* process linking a C library (confluent-kafka-go on Alpine, for instance).
We do the reverse: the Go runtime is a *guest* library inside a non-Go process
(your Rust binary), and it has to bootstrap itself from that host.
...
1. **The `*-musl` match arm in `build.rs`.** Today it is the honest failure:
   when the target's C environment is musl (`build.rs` keys on
   `CARGO_CFG_TARGET_ENV == "musl"`), the build panics immediately with a message
   that names the target, explains the `.init_array` argv/envp cause, and carries
   both issue links (golang/go#13492 with fix PR #69325, and golang/go#48596 for
   the dlopen TLS blocker), so a musl target produces a clear build error rather
   than a binary that segfaults at start. When the upstream fix lands, this arm
   becomes the CC and target-triple mapping for musl, exactly as the glibc arm
   maps the triple to `GOOS`/`GOARCH` and the C toolchain today.
2. **`ARG LIBC` in `docker/Dockerfile.builder`.** The builder image is
   parameterised on `LIBC`, which selects the base image and the Rust target.
   It defaults to `glibc` (a Debian base); the musl arm exits with the same
   upstream-blocker message the build.rs arm carries, so the Docker path is
   honest in the same way. Flipping musl on means giving this arm a real musl
   base image and Rust target instead of the early exit.
...
All three seams are landed and carry the same message; `build.rs` and
`docker/Dockerfile.builder` both point a reader here.
...
1. Pin the bridge's `go.mod` to a Go toolchain version that carries the fix, and
   rebuild the engine on musl to confirm the runtime now initialises.
2. Replace the `*-musl` early-failure arm in `build.rs` with the real musl
   target and C-toolchain mapping (mirroring the glibc arm), so a musl target
   triple builds instead of failing.
3. Replace the musl early-exit arm behind `ARG LIBC` in
   `docker/Dockerfile.builder` with a real musl base image and Rust target.
